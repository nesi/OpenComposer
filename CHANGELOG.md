...
- **First job submission failed and only succeeded on a second click.** The SSH key check ran
  only on Submit, so the very first submission raced the key it had just created. It now runs on
  the first page view of the app (`ensure_ssh_access`), well before anything needs the cluster.
  The check is memoized per process, and validated by two cheap stats so a `~/.ssh` that is
  wiped or restored mid-session is set up again on the next page view rather than staying broken
  until the PUN restarts.
- **An SSH key that was present but never authorized was treated as working.** The key check
  returned early on merely *finding* `~/.ssh/id_ed25519` (or any other candidate), which fails
  exactly like owning no key at all if it was never added to `~/.ssh/authorized_keys`. It now
  looks for a key that is actually authorized, and if none is, generates and authorizes one of
  its own. A key the user already owns is never authorized on their behalf. Keys whose `.pub` is
  missing are handled by deriving the public half with `ssh-keygen -y`; passphrase-protected keys
  are correctly treated as unusable rather than as working.
- **Empty Project dropdown gave no indication why.** The `sacctmgr` query that populates it runs
  over SSH to the login node, and every way that hop can fail (no usable key, no `known_hosts`
  entry, `sacctmgr` off the non-interactive `PATH`) was swallowed by `2>/dev/null` and a bare
  `rescue`, leaving an empty select and nothing to report. The reason is now shown in the field's
  help text. The query also no longer goes through a shell, which removes a quoting issue around
  the user name.
