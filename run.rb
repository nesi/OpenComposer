...
def show_website(job_id = nil, error_msg = nil, error_params = nil, script_path = nil)
  ...
  # Set up passwordless SSH to the login node on the first page view, so everything
  # that reaches the cluster later (the Project dropdown, submit, history, cancel)
  # already has a working key. Memoized per process; never fatal.
  ensure_ssh_access(@conf)
...
def ensure_user_ssh_key(key_type = "ed25519", key_name = "opencomposer")
  ...
  authorized = File.exist?(auth) ? File.read(auth) : ""

  # Having a key is not the same as having a *usable* key: a user may well own
  # ~/.ssh/id_ed25519 that was never added to authorized_keys, which fails exactly
  # like owning no key at all. So look for a key that is actually authorized, not
  # merely present.
  candidates = (%w[id_ed25519 id_rsa id_ecdsa id_dsa] +
                %w[ed25519 rsa ecdsa dsa].map { |t| "#{key_name}_#{t}" })
               .map { |k| File.join(ssh_dir, k) }
               .select { |k| File.exist?(k) }

  usable = candidates.find do |k|
    pub = ssh_public_key_for(k)
    pub && !pub.empty? && authorized.include?(pub)
  end
  return :exists if usable

  # Nothing authorized. Never authorize a pre-existing key on the user's behalf —
  # that is their key to manage — but do create and authorize one of our own.
  key_path = File.join(ssh_dir, base)
  result   = File.exist?(key_path) ? :authorized : :created
  ...
  # Authorize it for passwordless SSH to the (shared-home) login node.
  pub = ssh_public_key_for(key_path)
  return :failed if pub.nil? || pub.empty?
  File.open(auth, "a") { |f| f.puts(pub) } unless authorized.include?(pub)
...
# The public half of a private key: read the .pub beside it, else derive it with
# ssh-keygen -y. Returns nil for a passphrase-protected key (which is unusable for
# unattended SSH anyway) or if the key cannot be read.
def ssh_public_key_for(key_path)
  pub_path = "#{key_path}.pub"
  return File.read(pub_path).strip if File.exist?(pub_path)

  out, _err, status = Open3.capture3("ssh-keygen", "-y", "-P", "", "-f", key_path)
  status.success? ? out.strip : nil
rescue Exception
  nil
end

# Per-process memo of the SSH readiness check. A PUN serves exactly one user, but
# key by user anyway so a shared process could never leak one user's result to
# another. Only successes are remembered — a failure is retried on the next page
# view rather than being latched in until the PUN restarts.
SSH_ACCESS_STATE ||= {}

# Cheap validation of a remembered result: the memo is only trusted while the files it
# stands for are still there. Without this, a user whose ~/.ssh is wiped or restored
# mid-session would not be set up again until the PUN restarted, which on a long-lived
# PUN can be days. Two stats per page view is a fair price for the setup repairing
# itself; the memo still saves the file reads and any ssh-keygen calls.
def ssh_access_still_valid?
  ssh_dir = File.join(Dir.home, ".ssh")
  File.directory?(ssh_dir) && File.exist?(File.join(ssh_dir, "authorized_keys"))
end

# Make sure the user can actually SSH to the login node before anything needs to.
# Called on every page view of the app (cheap after the first, thanks to the memo)
# so that a user who has only ever opened Slurm Composer — never yet submitted —
# already has a working key. Previously this ran only on Submit, which meant the very
# first submission raced the key it had just created: users reported a failed submit
# that succeeded on the second click. It also left the Project dropdown (populated
# over SSH at form-render time) with nothing to authenticate with on a first visit.
def ensure_ssh_access(conf)
  return :disabled unless conf.fetch("ensure_ssh_key", false)

  user = ENV["USER"].to_s
  return SSH_ACCESS_STATE[user] if SSH_ACCESS_STATE.key?(user) && ssh_access_still_valid?

  result = begin
             ensure_user_ssh_key(conf.fetch("ssh_key_type", "ed25519"),
                                 conf.fetch("ssh_key_name", "opencomposer"))
           rescue Exception => e
             warn "[Open Composer] ensure_ssh_access failed for #{user}: #{e.class}: #{e.message}"
             :failed
           end

  SSH_ACCESS_STATE[user] = result unless result == :failed
  result
end
