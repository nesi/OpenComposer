...
// Recursively check ancestor toggle-checkboxes needed for `fieldKey` to be
// visible/enabled, using ocForm.enabledBy generated by form.rb.
ocForm.autoEnable = function(fieldKey, visited) {
  if (!ocForm.enabledBy) return;
  var baseKey  = fieldKey.replace(/_\d+$/, '');
  var enablers = ocForm.enabledBy[fieldKey] || ocForm.enabledBy[baseKey];
  if (!enablers) return;
  visited = visited || new Set();
  for (var j = 0; j < enablers.length; j++) {
    var enablerId = enablers[j];
    if (visited.has(enablerId)) continue;
    visited.add(enablerId);
    var enablerEl = document.getElementById(enablerId);
    if (!enablerEl) continue;
    var m = enablerId.match(/^(.+)_\d+$/);
    if (m) ocForm.autoEnable(m[1], visited);
    enablerEl.checked = true;
    ocForm.execDynamicWidget(enablerId);
  }
};
...
ocForm.parseScriptToWidgets = function(allowEnable) {
  ...
  for (const pat of ocForm.scriptLinePatterns) {
    ...
    for (var i = 0; i < pat.keys.length; i++) {
      ...
      case 'checkbox': {
        var sep  = pat.separators[i] || ',';
        var vals = value ? value.split(sep).map(function(v) { return v.trim(); }) : [];
        var n    = 1;
        while (true) {
...
ocForm.addSelectedItem = function(id) {
  ...
  // Allow any non-empty value (upstream v2.0.1 "custom multi-select values"),
  // not just entries in validSuggestions. addBadge() renders the badge from the
  // typed text and updateHiddenValues() submits anchor.textContent, so custom
  // values flow through even without a matching data-value.
